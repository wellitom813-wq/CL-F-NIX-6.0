(() => {
  ...
  const DEFAULT_CLANS=[{name:'CLÃ PRINCIPAL',tag:'#VJ8GGLR8',url:'https://link.clashofclans.com/pt?action=OpenClanProfile&tag=VJ8GGLR8',subtitle:'Família Fênix',enabled:true},{name:'CLÃ 2',tag:'#2GRGQVYQJ',url:'https://link.clashofclans.com/pt?action=OpenClanProfile&tag=2GRGQVYQJ',subtitle:'Família Fênix',enabled:true},{name:'CLÃ 3',tag:'#2G89YJVV8',url:'https://link.clashofclans.com/pt?action=OpenClanProfile&tag=2G89YJVV8',subtitle:'Família Fênix',enabled:true},{name:'CLÃ 4',tag:'#2CYY0VPUV',url:'https://link.clashofclans.com/pt?action=OpenClanProfile&tag=2CYY0VPUV',subtitle:'Família Fênix',enabled:true}];
  const DEFAULT_SECTION_ORDER=['hero','quick_links','clans','about','mission','resources','layouts','values','family'];
  const SECTION_META={hero:'Topo / apresentação',quick_links:'Links oficiais',clans:'Clãs Fênix',about:'Sobre o Clã',mission:'Missão',resources:'Cards de conteúdo',layouts:'Layouts',values:'Valores',family:'Chamada final'};
  ...
  const DEFAULT_CONFIG={
    ...
    about_kicker:'🔥 SOBRE O CLÃ',about_title:'Uma comunidade feita para evoluir em equipe.',about_text_1:'O Clã Fênix reúne jogadores de Clash of Clans que valorizam organização, respeito, participação e evolução constante.',about_text_2:'Nosso objetivo é manter uma família ativa, competitiva e acolhedora, onde cada membro contribui para fortalecer o grupo.',
    clans_kicker:'🏆 CLÃS FÊNIX',clans_title:'Família Fênix',clans_subtitle:'Acesse rapidamente todos os clãs da Família Fênix e abra direto no Clash of Clans.',clans:DEFAULT_CLANS,
    footer_motto:'Estratégia • União • Vitória',footer_disclaimer:'Clã Fênix é uma comunidade independente e não é afiliada à Supercell.',logo_url:'',logo_size:320,show_logo_caption:false,show_logo_decorations:false,card_radius:18,glow_strength:30,section_order:DEFAULT_SECTION_ORDER,show_clans:true,show_about:true,
  ...
  async function loadConfig(){
    ...
    if(!Array.isArray(siteConfig.resources))siteConfig.resources=[...DEFAULT_RESOURCES];if(!Array.isArray(siteConfig.values))siteConfig.values=[...DEFAULT_VALUES];if(!Array.isArray(siteConfig.clans))siteConfig.clans=DEFAULT_CLANS.map(x=>({...x}));if(!Array.isArray(siteConfig.section_order))siteConfig.section_order=[...DEFAULT_SECTION_ORDER];fillConfigForms();
  }
  function fillConfigForms(){
    ...
    setValue('cfgAboutKicker',c.about_kicker);setValue('cfgAboutTitle',c.about_title);setValue('cfgAboutText1',c.about_text_1);setValue('cfgAboutText2',c.about_text_2);setValue('cfgMissionIcon',c.mission_icon);setValue('cfgMissionKicker',c.mission_kicker);setValue('cfgMissionTitle',c.mission_title);setValue('cfgMissionText',c.mission_text);setValue('cfgResourcesKicker',c.resources_kicker);setValue('cfgResourcesTitle',c.resources_title);setValue('cfgResourcesSubtitle',c.resources_subtitle);setValue('cfgValuesKicker',c.values_kicker);setValue('cfgValuesTitle',c.values_title);setValue('cfgFamilyKicker',c.family_kicker);setValue('cfgFamilyTitle',c.family_title);setValue('cfgFamilyText',c.family_text);setValue('cfgFamilyQuote',c.family_quote);setValue('cfgFamilyCta',c.family_cta);setValue('cfgFooterMotto',c.footer_motto);setValue('cfgFooterDisclaimer',c.footer_disclaimer);setValue('cfgClansKicker',c.clans_kicker);setValue('cfgClansTitle',c.clans_title);setValue('cfgClansSubtitle',c.clans_subtitle);setValue('cfgLogoSize',c.logo_size||320);setValue('cfgGlowStrength',c.glow_strength??30);setValue('cfgCardRadius',c.card_radius??18);setChecked('showLogoCaption',c.show_logo_caption);setChecked('showLogoDecorations',c.show_logo_decorations);renderResourceEditor();renderValuesEditor();renderClansEditor();renderSectionOrderEditor();
    ...
    setColor('Primary',c.primary_color);setColor('Accent',c.accent_color);setColor('Background',c.background_color);updateAssetPreviews();
  }
  ...
  wireForm('contentForm',()=>({about_kicker:value('cfgAboutKicker'),about_title:value('cfgAboutTitle'),about_text_1:value('cfgAboutText1'),about_text_2:value('cfgAboutText2'),mission_icon:value('cfgMissionIcon'),mission_kicker:value('cfgMissionKicker'),mission_title:value('cfgMissionTitle'),mission_text:value('cfgMissionText'),resources_kicker:value('cfgResourcesKicker'),resources_title:value('cfgResourcesTitle'),resources_subtitle:value('cfgResourcesSubtitle'),resources:collectResources(),values_kicker:value('cfgValuesKicker'),values_title:value('cfgValuesTitle'),values:collectValues(),family_kicker:value('cfgFamilyKicker'),family_title:value('cfgFamilyTitle'),family_text:value('cfgFamilyText'),family_quote:value('cfgFamilyQuote'),family_cta:value('cfgFamilyCta'),footer_motto:value('cfgFooterMotto'),footer_disclaimer:value('cfgFooterDisclaimer')}),'Conteúdo atualizado!');
  ...
  function collectClans(){return [...$('clansEditor').querySelectorAll('.clan-edit-row')].map(row=>({name:row.querySelector('[data-field="name"]').value.trim(),tag:row.querySelector('[data-field="tag"]').value.trim(),url:row.querySelector('[data-field="url"]').value.trim(),subtitle:row.querySelector('[data-field="subtitle"]').value.trim(),enabled:row.querySelector('[data-field="enabled"]').checked}));}
  function renderClansEditor(){const arr=siteConfig.clans||DEFAULT_CLANS;$('clansEditor').innerHTML=arr.map((x,i)=>`<div class="clan-edit-row" data-index="${i}"><div class="clan-edit-head"><strong>${esc(x.name||`CLÃ ${i+1}`)}</strong><label class="enabled-inline"><input type="checkbox" data-field="enabled" ${x.enabled!==false?'checked':''}><span>Visível</span></label></div><div class="form-grid two"><label>Nome<input data-field="name" value="${esc(x.name||'')}"></label><label>Tag<input data-field="tag" value="${esc(x.tag||'')}"></label></div><label>Link do Clash<input data-field="url" type="url" value="${esc(x.url||'')}"></label><label>Texto pequeno<input data-field="subtitle" value="${esc(x.subtitle||'Família Fênix')}"></label></div>`).join('');}
  wireForm('clansForm',()=>({clans_kicker:value('cfgClansKicker'),clans_title:value('cfgClansTitle'),clans_subtitle:value('cfgClansSubtitle'),clans:collectClans()}),'Clãs Fênix atualizados!');

  const VIS_FIELD={hero:'show_hero',quick_links:'show_quick_links',clans:'show_clans',about:'show_about',mission:'show_mission',resources:'show_resources',layouts:'show_layouts',values:'show_values',family:'show_family'};
  function currentSectionOrderFromDom(){return [...$('sectionOrderEditor').querySelectorAll('.section-order-row')].map(r=>r.dataset.sectionKey);}
  function renderSectionOrderEditor(){const order=[...(siteConfig.section_order||DEFAULT_SECTION_ORDER)];DEFAULT_SECTION_ORDER.forEach(k=>{if(!order.includes(k))order.push(k)});$('sectionOrderEditor').innerHTML=order.map((key,i)=>{const field=VIS_FIELD[key];const visible=siteConfig[field]!==false;return `<div class="section-order-row" data-section-key="${key}"><div><span class="drag-index">${String(i+1).padStart(2,'0')}</span><strong>${esc(SECTION_META[key]||key)}</strong></div><label class="enabled-inline"><input type="checkbox" data-visible ${visible?'checked':''}><span>Visível</span></label><div class="move-group"><button class="icon-btn" type="button" data-section-up>↑</button><button class="icon-btn" type="button" data-section-down>↓</button></div></div>`}).join('');}
  $('sectionOrderEditor').addEventListener('click',e=>{const row=e.target.closest('.section-order-row');if(!row)return;const rows=[...$('sectionOrderEditor').children],i=rows.indexOf(row);if(e.target.closest('[data-section-up]')&&i>0)$('sectionOrderEditor').insertBefore(row,rows[i-1]);else if(e.target.closest('[data-section-down]')&&i<rows.length-1)$('sectionOrderEditor').insertBefore(rows[i+1],row);else return;[...$('sectionOrderEditor').querySelectorAll('.drag-index')].forEach((n,j)=>n.textContent=String(j+1).padStart(2,'0'));});
  wireForm('sectionsForm',()=>{const patch={section_order:currentSectionOrderFromDom()};$('sectionOrderEditor').querySelectorAll('.section-order-row').forEach(row=>{patch[VIS_FIELD[row.dataset.sectionKey]]=row.querySelector('[data-visible]').checked;});return patch;},'Seções e ordem atualizadas!');
  ...
  function updateAssetPreviews(){const heroBg=adminAssetUrl(siteConfig.hero_background_url),familyBg=adminAssetUrl(siteConfig.family_background_url),logo=adminAssetUrl(siteConfig.logo_url)||'../assets/fenix-f.png';$('heroBgPreview').style.backgroundImage=heroBg?`url("${heroBg}")`:'';$('familyBgPreview').style.backgroundImage=familyBg?`url("${familyBg}")`:'';if($('logoPreviewImg'))$('logoPreviewImg').src=logo;}
  ...
  previewBackground($('cfgHeroBgFile'),'heroBgPreview');previewBackground($('cfgFamilyBgFile'),'familyBgPreview');$('cfgLogoFile')?.addEventListener('change',()=>{const f=$('cfgLogoFile').files[0];if(f&&$('logoPreviewImg'))$('logoPreviewImg').src=URL.createObjectURL(f);});$('btnResetLogo')?.addEventListener('click',()=>{siteConfig.logo_url='';if($('cfgLogoFile'))$('cfgLogoFile').value='';if($('logoPreviewImg'))$('logoPreviewImg').src='../assets/fenix-f.png';});
  ...
  $('appearanceForm').addEventListener('submit',async e=>{
    ...
    try{
      let hero_background_url=siteConfig.hero_background_url,family_background_url=siteConfig.family_background_url,logo_url=siteConfig.logo_url;
      const heroFile=$('cfgHeroBgFile').files[0],familyFile=$('cfgFamilyBgFile').files[0],logoFile=$('cfgLogoFile').files[0];
      ...
      if(logoFile)logo_url=await uploadSiteAsset(logoFile,'logo');
      const ok=await saveConfig({hero_background_url,family_background_url,logo_url,logo_size:Number(value('cfgLogoSize')||320),glow_strength:Number(value('cfgGlowStrength')||30),card_radius:Number(value('cfgCardRadius')||18),show_logo_caption:checked('showLogoCaption'),show_logo_decorations:checked('showLogoDecorations'),primary_color:value('cfgPrimaryColorText'),accent_color:value('cfgAccentColorText'),background_color:value('cfgBackgroundColorText')},'Aparência atualizada!');
      if(ok){$('cfgHeroBgFile').value='';$('cfgFamilyBgFile').value='';$('cfgLogoFile').value='';updateAssetPreviews();}
